      ...
      if request.method == "POST":
            ...
            if form.is_valid():
                  ...
                  if user is not None:         #si el usario ingresado no es None (quiere decir que existe), listo, pedir que se loguee
                        login(request, user)
                        return render(request,"empresa/home.html",  {"mensaje":f"Bienvenido a la empresa, {usuario}"})   #si es correcto, loguea y me responde welcome
                  else:
                        return render(request,"Users/login.html", {"form":form, "mensaje":"Error, el username es incorrecto. Por favor, intentalo nuevamente."})   #y si el usuario que ingresó no corresponde a nadie en la BD, me pasa que es incorrecto

            else:
                        return render(request,"Users/login.html" ,  {"form":form, "mensaje":"Error, la informacion es incorrecta. Por favor, intentalo nuevamente."})  #esto se hace si no se cumple la condicion de que el formulario sea valido
...
def register(request):
      ...
      if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                return render(request, 'Users/register.html', {"form":form, "mensaje": "El usuario no pudo ser creado. Por favor intentalo nuevamente."})
